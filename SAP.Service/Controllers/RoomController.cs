...
namespace SAP.Service.Controllers
{
    ...
    {
        ITypeBusinessService<Room> service;

        public RoomController(IHttpContextAccessor httpContextAccessor, IConfiguration configuration,
             ITypeBusinessService<Room> service) : base(httpContextAccessor, configuration)
        {
        ...
        {
            var result = service.GeneratePdf("Lista de Salas Registrados");
            SaveRequest(response: result);
            return result;
        }
    }
