...
namespace SAP.Service.Controllers
{
    ...
    public class SexTypeController : BaseController
    {
        ITypeBusinessService<SexType> service;

        public SexTypeController(IHttpContextAccessor httpContextAccessor, IConfiguration configuration,
            ITypeBusinessService<SexType> service) : base(httpContextAccessor, configuration)
        {
        ...
        {
            var result = service.GeneratePdf("Lista de Generos Registrados");
            SaveRequest();
            return result;
        }
    }
