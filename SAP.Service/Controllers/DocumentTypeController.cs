...
namespace SAP.Service.Controllers
{
    ...
    {
        ITypeBusinessService<DocumentType> service;

        public DocumentTypeController(IHttpContextAccessor httpContextAccessor, IConfiguration configuration,
                    ITypeBusinessService<DocumentType> service) : base(httpContextAccessor, configuration)
        {
        ...
        {
            var result = service.GeneratePdf("Lista de Tipos de Documento Registrados");
            SaveRequest(response: result);
            return result;
        }
    }
