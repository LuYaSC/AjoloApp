...
namespace AjoloApp.Service.Controllers
{
    ...
    {
        ITypeBusinessService<BloodType> service;

        public BloodTypeController(IHttpContextAccessor httpContextAccessor, IConfiguration configuration,
                     ITypeBusinessService<BloodType> service) : base(httpContextAccessor, configuration)
        {
        ...
        {
            var result = service.GeneratePdf("Lista de Tipos de Sangre Registrados");
            SaveRequest(response: result);
            return result;
        }
    }
