...
namespace AjoloApp.Service.Controllers
{
    ...
    {
        ITypeBusinessService<PaymentType> service;

        public PaymentTypeController(IHttpContextAccessor httpContextAccessor, IConfiguration configuration,
                    ITypeBusinessService<PaymentType> service) : base(httpContextAccessor, configuration)
        {
        ...
        {
            var result = service.GeneratePdf("Lista de Tipos de Pagos Registrados");
            SaveRequest(response: result);
            return result;
        }
    }
